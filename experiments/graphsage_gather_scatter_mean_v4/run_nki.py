"""
GraphSAGE gather+mean — NKI (v4: src-block grouped neighbor ordering).

This script matches `run_torch.py` in how it constructs the CPU-side padded
destination segments:
  - after sorting edges by `dst`, neighbors inside each destination segment
    are reordered by `src_block = src // SRC_BLOCK_SIZE`.

The NKI kernel then computes:
  out[n] = sum_k x[src_padded[n*MAX_DEG+k], :] * mask_padded[n*MAX_DEG+k] / degree[n]
where `mask_padded` is 0/1 for padded slots.
"""
...
def build_src_padded(src: torch.Tensor, dst: torch.Tensor, num_nodes: int, max_deg: int):
    """Build padded destination segments, reordered by src_block."""
...
def gather_mean_nki(x, src_padded, mask_padded, inv_degree):
    """Option 2 fused gather+mask+segment sum+mean (as in v2)."""
    ...
    for nt in nl.affine_range(num_node_tiles):
        ...
        for c in nl.affine_range(MAX_DEG // D_CHUNK):
            ...
            for dk in nl.affine_range(D_CHUNK):
                ...
                m_dk = nl.load(mask_padded[row_idx])        # [TILE_NODES, 1]

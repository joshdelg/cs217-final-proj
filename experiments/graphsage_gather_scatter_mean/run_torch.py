...
To make a fair comparison with an NKI kernel that fuses gather too, this
experiment uses a padded "destination segment" representation:

  - Sort edges by destination (`dst`) on CPU.
  - Build `src_padded` and `mask_padded` so that for each node n:
        seg_values[k] = x[src_padded[n*MAX_DEG + k]] * mask_padded[n*MAX_DEG + k]
    and the aggregation is:
        out[n] = sum_k seg_values[k] / degree[n]

Inside the profiled region, PyTorch performs:
  `messages = x[src_padded]`, then `segment_sum` and `mean`.
"""
...
def generate_random_graph(num_nodes: int, avg_degree: int, seed: int = 42):
    ...
    degree.clamp_(min=1.0)  # avoid div-by-zero for mean
...
def build_src_padded(src: torch.Tensor, dst: torch.Tensor, degree: torch.Tensor, num_nodes: int, max_deg: int):
    ...
    Returns:
        src_padded:  [num_nodes * max_deg] int64 (padded with 0 indices)
        mask_padded: [num_nodes * max_deg] float32 (1 for valid edges else 0)
    """
    # Sort by destination so edges for each dst are contiguous.
    perm = dst.argsort(stable=True)
    src_sorted = src[perm]
    dst_sorted = dst[perm]

    # CSR rowptr for contiguous destination segments.
...
def main():
    ...
    # Compute padded MAX_DEG = next power-of-two of in-degree.
    ...
    # Gather inside the profiled NEFF
    ...
    out = agg * inv_degree[:, None]  # mean normalization
    xm.mark_step()
    xm.wait_device_ops()

    # Materialize output on host to prevent dead-code elimination.

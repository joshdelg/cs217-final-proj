"""
2-layer GCN with NKI aggregation + PyTorch linear layers.

We map GCN aggregation to the same NKI kernel used for GraphSAGE:
  out[n] = (sum_k x[src_padded[n*MAX_DEG+k], :] * mask_padded[n*MAX_DEG+k]) * inv_degree[n]

For GCN:
  - mask_padded stores symmetric normalized edge weights
      1 / sqrt(deg(src) * deg(dst))  for A_hat = A + I
  - inv_degree is all ones
"""
...
def gcn_aggregate_torch_cpu(x: torch.Tensor, src_padded: torch.Tensor, norm_padded: torch.Tensor, inv_degree: torch.Tensor, max_deg: int):
    messages = x[src_padded.long()] * norm_padded[:, None]
...
def gather_mean_nki(x, src_padded, mask_padded, inv_degree):
    """v1/Option-1 gather+reduce kernel reused across architectures."""
...
def main():
    ...
    # CPU reference.
    with torch.no_grad():
        agg1 = gcn_aggregate_torch_cpu(x0, src_padded_cpu, norm_padded_cpu, inv_degree_cpu, MAX_DEG)
        h1 = F.linear(agg1, W1, b1)
        h1 = F.relu(h1)
        agg2 = gcn_aggregate_torch_cpu(h1, src_padded_cpu, norm_padded_cpu, inv_degree_cpu, MAX_DEG)
    ...
    # ---- profiled forward ----
    agg1 = gather_mean_nki(x, src_padded, norm_padded, inv_degree)
    h1 = F.linear(agg1, W1, b1)
    h1 = F.relu(h1)
    agg2 = gather_mean_nki(h1, src_padded, norm_padded, inv_degree)
